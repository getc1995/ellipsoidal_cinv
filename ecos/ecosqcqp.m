function [X,fval,exitflag,output,lambda,Tsolve,c,G,h,dims,Aeq,beq] = ecosqcqp(H,f,A,B,Aeq,Beq,ellip_list,opts)
% Revised based on ECOSQP by Tiancheng Ge at Umich. Add More ellipsoidal constraints

% H has to be semi-positive-definite, otherwise the program will 
% return 0 for the optimal objective.
...
zerorow = zeros(1,size(G,2));
% |F(x-c)|_2 <= 1
% Fx - Fc >=_K 0
% G = -F, h = -Fc

for i = 1:length(ellip_list)
    ellip = ellip_list{i};
    zerocolumn = zeros(size(ellip.F, 1),1);
    G = [G; 
        zerorow;
        -ellip.F, zerocolumn;];
    h = [h; 
        1;
        -ellip.Fc;];
    dims.q = [dims.q; length(ellip.Fc)+1];
end
